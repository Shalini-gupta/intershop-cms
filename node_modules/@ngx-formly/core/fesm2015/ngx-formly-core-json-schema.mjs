...
import { ɵisNil, ɵgetFieldValue, ɵreverseDeepMerge, ɵclone } from '@ngx-formly/core';
...
function totalMatchedFields(field) {
    if (!field.fieldGroup) {
        return !ɵisNil(field.key) && ɵgetFieldValue(field) !== undefined ? 1 : 0;
    }
    const total = field.fieldGroup.reduce((s, f) => totalMatchedFields(f) + s, 0);
    return total === 0 ? (field.key && ɵgetFieldValue(field) !== undefined ? 1 : 0) : total;
}
class FormlyJsonschema {
        ...
        switch (field.type) {
            ...
            case 'object': {
                ...
                Object.keys(schema.properties || {}).forEach((property) => {
                    const f = this._toFieldConfig(schema.properties[property], Object.assign(Object.assign({}, options), { key: property }));
                    field.fieldGroup.push(f);
                    if ((Array.isArray(schema.required) && schema.required.indexOf(property) !== -1) || propDeps[property]) {
                        f.expressionProperties = Object.assign(Object.assign({}, (f.expressionProperties || {})), { 'templateOptions.required': (m, s, f) => {
                                let parent = f.parent;
                    ...
                    if (schemaDeps[property]) {
                        ...
                        if (oneOfSchema &&
                            ...
                            oneOfSchema.forEach((oneOfSchemaItem) => {
                                ...
                                field.fieldGroup.push(Object.assign(Object.assign({}, this._toFieldConfig(Object.assign(Object.assign({}, oneOfSchemaItem), { properties }), Object.assign(Object.assign({}, options), { resetOnHide: true }))), { hideExpression: (m) => !m || getConstValue(constSchema) !== m[property] }));
                            });
                        }
                        else {
                            field.fieldGroup.push(Object.assign(Object.assign({}, this._toFieldConfig(schemaDeps[property], options)), { hideExpression: (m) => !m || isEmpty(m[property]) }));
                        }
                    }
                });
                if (schema.oneOf) {
    ...
    resolveMultiSchema(mode, schemas, options) {
        return {
            ...
            fieldGroup: [
                ...
                {
                    fieldGroup: schemas.map((s, i) => (Object.assign(Object.assign({}, this._toFieldConfig(s, Object.assign(Object.assign({}, options), { resetOnHide: true }))), { hideExpression: (m, fs, f, forceUpdate) => {
    ...
    resolveDefinition(schema, options) {
        ...
        const definition = !pointer
            ? null
            : pointer.split('/').reduce((def, path) => ((def === null || def === void 0 ? void 0 : def.hasOwnProperty(path)) ? def[path] : null), options.schema);
        if (!definition) {
    ...
    isFieldValid(field, schema, options) {
        const { form } = field.options.build({
            form: new FormGroup({}),
            fieldGroup: [this._toFieldConfig(schema, Object.assign(Object.assign({}, options), { resetOnHide: true, ignoreDefault: true, map: null }))],

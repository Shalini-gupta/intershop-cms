...
exports.updateComponentDecorator = exports.updateComponentClassName = exports.updateComponentSelector = exports.move = void 0;
        ...
        host.visit(file => {
            if (file.startsWith(`/src/app/`) || file.startsWith(`/projects/`)) {
                ...
                if (file.endsWith('.ts')) {
                    ...
                    const imports = (0, tsquery_1.tsquery)((0, filesystem_1.readIntoSourceFile)(host, file), file.includes(fromName) ? `ImportDeclaration` : `ImportDeclaration[text=/.*${fromName}.*/]`).filter((x) => file.includes(fromName) || x.getText().includes(`/${fromName}/`));
                    if (imports.length) {
                        const updates = [];
...
function updateComponentClassName(host, file, fromClassName, toClassName) {
    const identifiers = (0, tsquery_1.tsquery)((0, filesystem_1.readIntoSourceFile)(host, file), `Identifier[name=${fromClassName}]`);
...
function updateComponentDecorator(host, file, fromName, toName) {
    const updater = host.beginUpdate(file);
    (0, tsquery_1.tsquery)((0, filesystem_1.readIntoSourceFile)(host, file), 'Decorator Identifier[name=Component]')
        .map(x => x.parent)
        .forEach(componentDecorator => {
        (0, tsquery_1.tsquery)(componentDecorator, 'PropertyAssignment')

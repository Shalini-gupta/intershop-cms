...
function adoptImportStatements(host, file) {
    ...
    (0, tsquery_1.tsquery)(source, 'ImportDeclaration > StringLiteral').forEach(x => {
        const newImport = x
            .getText()
...
function transformContainerComponentIdentifiers(host, file) {
    let update;
    const source = (0, filesystem_1.readIntoSourceFile)(host, file);
    (0, tsquery_1.tsquery)(source, 'Identifier[name=/.*ContainerComponent$/]')
...
function deContainerizeComponentDecorator(host, file) {
    let update;
    const source = (0, filesystem_1.readIntoSourceFile)(host, file);
    (0, tsquery_1.tsquery)(source, 'Decorator > CallExpression > Identifier[name="Component"]')
        .map(x => x.parent)
        .forEach((x) => {
